...
def add_user():
    ...
    # Code review: Without the "or None" it was passing an empty string and the model needs a Null to put the template image
...
def show_user_details(user_id):
    ...
    # Code Review: .get returns the page with all the values as "None" but still showing the template HTML. Also one click away from a bug if users try to delete the non-existing user
    user = User.query.get_or_404(user_id)
    
    posts = user.posts
    # posts = Post.query.filter(user_id == user.id)
...
def add_post(user_id):
    ...
    flash(f"'{title}' was successfully added.")
